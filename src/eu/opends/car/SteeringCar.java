...
import se.christiannils.safetysystems.SafetySystems;
import se.christiannils.safetysystems.SafetySystemOutput;
/**
 * Driving Car
 * 
 * @author Rafael Math
...
public class SteeringCar extends Car implements TrafficObject
{
	...
	// Safety systems
    private SafetySystems safetySystems;
    private SafetySystemOutput safetySystemOutput;
	...
	{		
        ...
        // CN: initialize safety systems
        safetySystems = new SafetySystems(sim, this, true, false);
	...
	public void update(float tpf, ArrayList<TrafficObject> vehicleList)
	{
		...
		if(!isAutoPilot)
		{
			// CN: update safety systems
	        safetySystemOutput = safetySystems.update(tpf);
        	// activate warning
	        PanelCenter.showWarningFrame(safetySystemOutput.isWarning(), 100);	        
			...
			else if(safetySystemOutput.getBrakePedal()!=0){ //CN
				pAccel = powerTrain.getPAccel(tpf, 0) * 30f;
			}
			...
			else if(safetySystemOutput.getBrakePedal()!=0){
				// hand brake
				carControl.brake(maxBrakeForce*safetySystemOutput.getBrakePedal()); //if brakePedal=1, then brake as much as the maxBrakeForce
			}
			else
			{
				// brake	
